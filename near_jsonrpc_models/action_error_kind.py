...
class ActionErrorKindGasKeyBalanceTooHighPayload(BaseModel):
    account_id: AccountId
    balance: NearToken
    # Set for DeleteKey (specific key), None for DeleteAccount (aggregate)
    public_key: PublicKey | None = None

class ActionErrorKindGasKeyBalanceTooHigh(StrictBaseModel):
    """Gas key balance is too high to burn during deletion"""
    GasKeyBalanceTooHigh: ActionErrorKindGasKeyBalanceTooHighPayload

class ActionErrorKindDelegateActionInvalidNonceIndexPayload(BaseModel):
    nonce_index: conint(ge=0, le=65535)
    num_nonces: conint(ge=0, le=65535)

class ActionErrorKindDelegateActionInvalidNonceIndex(StrictBaseModel):
    """DelegateAction nonce index is outside the gas key's nonce range"""
    DelegateActionInvalidNonceIndex: ActionErrorKindDelegateActionInvalidNonceIndexPayload

class ActionErrorKindTotalPromiseInputSizeExceededPayload(BaseModel):
    limit: conint(ge=0, le=18446744073709551615)
    size: conint(ge=0, le=18446744073709551615)

class ActionErrorKindTotalPromiseInputSizeExceeded(StrictBaseModel):
...
referenced by the receipt's `input_data_ids`) exceeded the limit."""
    TotalPromiseInputSizeExceeded: ActionErrorKindTotalPromiseInputSizeExceededPayload

class ActionErrorKindReceiptStorageProofSizeExceededPayload(BaseModel):
    limit: conint(ge=0, le=18446744073709551615)

class ActionErrorKindReceiptStorageProofSizeExceeded(StrictBaseModel):
    """The receipt recorded more storage proof than
`per_receipt_storage_proof_size_limit` allows."""
    ReceiptStorageProofSizeExceeded: ActionErrorKindReceiptStorageProofSizeExceededPayload
...
class ActionErrorKindMalformedUniversalStateInit(RootModel[Literal['MalformedUniversalStateInit']]):
    pass

class ActionErrorKindAccountNotInitializedPayload(BaseModel):
    account_id: AccountId

class ActionErrorKindAccountNotInitialized(StrictBaseModel):
